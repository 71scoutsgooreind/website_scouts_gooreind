          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              {group.members.map((member, index) => (
                <div
                  key={member.name}
                  className="bg-green-50 rounded-xl shadow-lg overflow-hidden transform hover:scale-105 transition-transform duration-300 flex flex-col animate-card"
                  style={{ animationDelay: `${index * 100}ms` }}
                >
                  {/* Afbeelding met vaste verhouding */}
                  <div className="w-full aspect-square bg-green-200 flex items-center justify-center overflow-hidden">
                    {member.image ? (
                      <img
                        src={member.image}
                        alt={member.name}
                  ...
                  <div className="p-4 flex-grow flex flex-col">
                    ...
                    <p className="text-md text-green-700 italic mb-4">
                      {member.scoutsName}
                    </p>
